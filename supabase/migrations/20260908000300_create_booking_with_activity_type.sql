-- Acrescenta o tipo de atividade a create_booking. Mesma estrutura das versões
-- anteriores; os drops removem as assinaturas antigas para não criar sobrecarga.
...
drop function if exists public.create_booking(
  uuid, uuid, uuid, text, timestamptz, timestamptz, uuid[], uuid
);
...
create or replace function public.create_booking(
  ...
  p_resource_ids uuid[],
  p_activity_type public.activity_type default 'class',
...
begin
  ...
  values (
    p_room_id, p_professor_id, p_class_id, p_subject_id, p_activity_type,
